...
| 없는 것 | 결과 |
|---|---|
| 인증 | 포트에 닿는 누구나 148.7 GiB 모델을 무료로 사용 |
...
**1) 재시작 비용 차이.** LiteLLM은 약 30초, vLLM은 **5~6분**(가중치 148.7 GiB 로딩 + CUDA 그래프 캡처)입니다. "모델 하나 추가" 같은 변경이 6분 재시작을 유발하는 상황을 만들지 않아야 해요.
...
이 저장소 구성에서 vLLM을 2대로 늘리려면 가중치 148.7 GiB × 2 = **GPU 4장**이 필요합니다.
